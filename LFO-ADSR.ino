...
void handleParameter(int paramChoice) {
  ...
  switch (quantChoice) {
    case LFO_RATE_PARAMETER_VALUE:
      lfoPeriodLength = map(paramValue, 0, 1024, LFO_PERIOD_LOW, LFO_PERIOD_HIGH); 
      break;
    ...
    case ADSR_ATTACK_PARAMETER_VALUE:
      adsrAttackTime = map(paramValue, 0, 1024, ADSR_TIME_LOW, ADSR_TIME_HIGH);
      break;
    case ADSR_DECAY_PARAMETER_VALUE:
      adsrDecayTime = map(paramValue, 0, 1024, ADSR_TIME_LOW, ADSR_TIME_HIGH);
      break;
    case ADSR_SUSTAIN_PARAMETER_VALUE:
      adsrSustainLevel = map(paramValue, 0, 1024, 0, 256);
      break;
    case ADSR_RELEASE_PARAMETER_VALUE:
      adsrReleaseTime = map(paramValue, 0, 1024, ADSR_TIME_LOW, ADSR_TIME_HIGH);
      break;
